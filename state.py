...
class State:
    ...
    def __init__(self, board, predecessor, move):
        ...
        fields/attributes:
        - a board object for this state
        - a state object that comes before this state in the current sequence
        - string move that was used to get to this current state
        - number of moves needed from initial state to get to this state
        """
        self.board = board
    ...
    def generate_successors(self):
        ...
        successors = []
        for i in MOVES:
            new = Board(self.board.digit_string())
            if new.move_blank(i) == True:
                successor = State(new, self, i)
    ...
    def print_moves_to(self):
        ...
        called State object (i.e., to self).
        """
        if self.num_moves == 0:
            print('initial state:')
            print(self.board)
